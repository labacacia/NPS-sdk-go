...
| `nwp` | NWP | QueryFrame, ActionFrame, NwpClient (HTTP mode); `ErrAuth*` / `ErrQuery*` / … error code constants (`nwp/error_codes.go`) |
...
| `ndp` | NDP | AnnounceFrame, ResolveFrame, GraphFrame, InMemoryNdpRegistry, NdpAnnounceValidator; DNS TXT fallback (`ResolveViaDns`, `DnsTxtLookup`, `ParseNpsTxtRecord`) |
